...
# 데이터 불러오기
hr_df=pd.read_csv('/data1/CHH/현대모비스/정형데이터/hr_df.csv')

# 데이터 프레임 및 하이퍼 파라미터 
hr_list=[(hr_df, 2, 'tanh','relu', None, 'relu',30,30,10,10,'adam',0.07,10,4),
(hr_df, 4, 'relu',None, 'relu', 'sigmoid',30,30,10,10,'adam',0.07,10,4),
(hr_df, 7, 'relu','sigmoid', 'tanh', 'relu',30,30,10,10,'adam',0.07,10,4)]
    # (2,hr_df_word, 256,30, 'relu','relu','sgd'),
    # (4,hr_df_word, 256,30, 'relu','relu','sgd'),
    # (7,hr_df_word, 256,30, 'relu','relu','sgd')]
...
val_mape_list=[]
for change_df, month_s, lstm_1,lstm_2,den_1,den_2, lstm_num1, lstm_num2,  den_num1, den_num2, opti, learning_rate_num, batch_num, nstep in hr_list:
    ...
    vhr_x=dat.iloc[-(month_s*4)-(nstep-4):,:-1]
    vhr_y=dat.iloc[-(month_s*4)-(nstep-4):,-1]
    ...
    vhr_x.reset_index(inplace=True,drop=True)
    vhr_y.reset_index(inplace=True,drop=True)

    vhr_total=pd.concat([vhr_x,vhr_y],axis=1)
...
# result_save.to_csv('./Result/hr_predict_values.csv',encoding='utf-8-sig',index=False)
hr = result_save.copy()
hr_mape=val_mape_list
# hr_analysis()
print('Done!')