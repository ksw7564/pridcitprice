...
# 데이터 불러오기
ni_df=pd.read_csv('/data1/CHH/현대모비스/정형데이터/nickel_datav3.csv')

# 데이터 프레임 및 하이퍼 파라미터 
ni_list=[(ni_df, 2, 'relu',None, 'sigmoid', 'sigmoid',30,200,30,30,'adam',0.07,50,20),
(ni_df, 4, 'relu',None, 'sigmoid', 'sigmoid',30,200,30,30,'adam',0.07,50,20),
(ni_df, 7, 'relu',None, 'sigmoid', 'sigmoid',30,200,30,30,'adam',0.07,50,20)]

    # (2,ni_df_word, 256,30, 'relu','relu','sgd'),
    # (4,ni_df_word, 256,30, 'relu','relu','sgd'),
    # (7,ni_df_word, 256,30, 'relu','relu','sgd')]
...
val_mape_list=[]
for change_df, month_s, lstm_1,lstm_2,den_1,den_2, lstm_num1, lstm_num2,  den_num1, den_num2, opti, learning_rate_num, batch_num, nstep in ni_list:
...
# result_save.to_csv('./Result/ni_predict_values.csv',encoding='utf-8-sig',index=False)
ni = result_save.copy()
ni_mape=val_mape_list
# ni_analysis()
print('Done!')