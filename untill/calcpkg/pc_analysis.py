...
# 데이터 불러오기
pc_df=pd.read_csv('/data1/CHH/현대모비스/정형데이터/weekly_pc_final.csv',encoding='cp949')

# 데이터 프레임 및 하이퍼 파라미터 
pc_list=[(pc_df, 2, None,'relu', 'relu', 'sigmoid',30,30,10,10,'adam',0.07,10,4),
(pc_df, 4, 'relu','relu', 'relu', 'sigmoid',30,50,30,30,'adam',0.07,10,4),
(pc_df, 7, 'relu','sigmoid', 'relu', 'relu',30,30,10,10,'adam',0.07,10,4)]
    # (2,pc_df_word, 256,30, 'relu','relu','sgd'),
    # (4,pc_df_word, 256,30, 'relu','relu','sgd'),
    # (7,pc_df_word, 256,30, 'relu','relu','sgd')]
...
val_mape_list=[]
for change_df, month_s, lstm_1,lstm_2,den_1,den_2, lstm_num1, lstm_num2,  den_num1, den_num2, opti, learning_rate_num, batch_num, nstep in pc_list:
    ...
    vpc_x=dat.iloc[-(month_s*4)-(nstep-4):,:-1]
    vpc_y=dat.iloc[-(month_s*4)-(nstep-4):,-1]
    ...
    vpc_x.reset_index(inplace=True,drop=True)
    vpc_y.reset_index(inplace=True,drop=True)

    vpc_total=pd.concat([vpc_x,vpc_y],axis=1)
...
# result_save.to_csv('./Result/pc_predict_values.csv',encoding='utf-8-sig',index=False)
pc = result_save.copy()
pc_mape=val_mape_list
# pc_analysis()
print('Done!')