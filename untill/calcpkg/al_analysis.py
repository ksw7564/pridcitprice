...
# /from keras.callbacks import ModelCheckpointP
...
# 데이터 불러오기
al_df=pd.read_csv('/data1/CHH/현대모비스/정형데이터/aluminum_datav2.csv')

# 데이터 프레임 및 하이퍼 파라미터 
al_list=[(al_df, 2, 'relu','tanh', None, 'relu',30,30,30,30,'adam',0.07,50,20),
(al_df, 4, 'relu','tanh', None, 'relu',30,30,30,30,'adam',0.07,50,20),
(al_df, 7, 'relu','tanh', None, 'relu',30,30,30,30,'adam',0.07,50,20)]
    # (2,al_df_word, 256,30, 'relu','relu','sgd'),
    # (4,al_df_word, 256,30, 'relu','relu','sgd'),
    # (7,al_df_word, 256,30, 'relu','relu','sgd')]
...
val_mape_list=[]
sangwoo=0
for change_df, month_s, lstm_1,lstm_2,den_1,den_2, lstm_num1, lstm_num2,  den_num1, den_num2, opti, learning_rate_num, batch_num, nstep in al_list:

    change_df.reset_index(drop=True,inplace=True)
    dat=change_df.copy()

    yy=dat[dat.columns[1]].shift((month_s-1)*-20)
    ...
    train_x=dat.iloc[:-(month_s*20)-(nstep-20),:-1]
    train_y=dat.iloc[:-(month_s*20)-(nstep-20),-1]
    val_x=dat.iloc[-(month_s*20)-(nstep-20):,:-1]
    val_y=dat.iloc[-(month_s*20)-(nstep-20):,-1]
    test_x=change_df.iloc[-(month_s*20)-(nstep-20):,1:]
    test_y=change_df.iloc[-(month_s*20)-(nstep-20):,1]
    ...
    val_total=pd.concat([val_x,val_y],axis=1)
    ...
    price_pred=np.concatenate((dff[:20*(month_s-1),:dff.shape[1]-1],y_all_predict),axis=1)
...
# result_save.to_csv('./Result/al_predict_values.csv',encoding='utf-8-sig',index=False)
al = result_save.copy()
al_mape=val_mape_list
